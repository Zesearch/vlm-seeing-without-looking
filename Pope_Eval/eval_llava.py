...
"""
LLaVA-1.5 evaluation script with Residual Injection
Adapted from original perturbation test
"""
...
from residual_hook import ResidualInjectionHook
...
# ============================================================
# 4) LLaVA inference (traditional format)
# ============================================================
@torch.inference_mode()
def infer_llava_batch(model, processor, images_pil, questions, max_new_tokens=8):
    """
    Batch inference for LLaVA-1.5 (traditional processor format)
    """
    assert len(images_pil) == len(questions)

    # LLaVA 1.5 traditional prompt format
...
def infer_with_image_batch(model, processor, images, questions, max_new_tokens=8):
    return infer_llava_batch(model, processor, images, questions, max_new_tokens=max_new_tokens)
...
# 8) Model loading
# ============================================================
def load_llava_local(model_path: str):
    print("="*80)
...
def main():
    ...
    # Load model
    model, processor = load_llava_local(args.model_path)
